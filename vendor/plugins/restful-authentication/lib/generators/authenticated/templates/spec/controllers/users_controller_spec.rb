...
<% if options[:include_activation] -%>
  ...
  it 'requires login on signup' do
    lambda do
      ...
      assigns[:<%= file_name %>].errors[:login].should_not be_empty
  ...
  it 'requires password on signup' do
    lambda do
      ...
      assigns[:<%= file_name %>].errors[:password].should_not be_empty
      response.should be_success
    end.should_not change(<%= class_name %>, :count)
  end
  ...
  it 'requires password confirmation on signup' do
    lambda do
      ...
      assigns[:<%= file_name %>].errors[:password_confirmation].should_not be_empty
      response.should be_success
    end.should_not change(<%= class_name %>, :count)
  end
  ...
  it 'requires email on signup' do
    lambda do
      ...
      assigns[:<%= file_name %>].errors[:email].should_not be_empty
      response.should be_success
    end.should_not change(<%= class_name %>, :count)
  end
...
describe <%= model_controller_class_name %>Controller do
  ...
  describe "route recognition" do
    it "should generate params for <%= model_controller_controller_name %>'s index action from GET /<%= model_controller_routing_path %>" do
      {:get => '/<%= model_controller_routing_path %>'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'index')
      {:get => '/<%= model_controller_routing_path %>.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'index', :format => 'xml')
      {:get => '/<%= model_controller_routing_path %>.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'index', :format => 'json')
    end
    
    it "should generate params for <%= model_controller_controller_name %>'s new action from GET /<%= model_controller_routing_path %>" do
      {:get => '/<%= model_controller_routing_path %>/new'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'new')
      {:get => '/<%= model_controller_routing_path %>/new.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'new', :format => 'xml')
      {:get => '/<%= model_controller_routing_path %>/new.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'new', :format => 'json')
    end
    ...
    it "should generate params for <%= model_controller_controller_name %>'s create action from POST /<%= model_controller_routing_path %>" do
      {:post => '/<%= model_controller_routing_path %>'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'create')
      {:post => '/<%= model_controller_routing_path %>.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'create', :format => 'xml')
      {:post => '/<%= model_controller_routing_path %>.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'create', :format => 'json')
    end
    
    it "should generate params for <%= model_controller_controller_name %>'s show action from GET /<%= model_controller_routing_path %>/1" do
      {:get  => '/<%= model_controller_routing_path %>/1'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'show', :id => '1')
      {:get  => '/<%= model_controller_routing_path %>/1.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'show', :id => '1', :format => 'xml')
      {:get  => '/<%= model_controller_routing_path %>/1.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'show', :id => '1', :format => 'json')
    end
    ...
    it "should generate params for <%= model_controller_controller_name %>'s edit action from GET /<%= model_controller_routing_path %>/1/edit" do
      {:get  => '/<%= model_controller_routing_path %>/1/edit'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'edit', :id => '1')
    end
    ...
    it "should generate params {:controller => '<%= model_controller_controller_name %>', :action => update', :id => '1'} from PUT /<%= model_controller_routing_path %>/1" do
      {:put  => '/<%= model_controller_routing_path %>/1'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'update', :id => '1')
      {:put  => '/<%= model_controller_routing_path %>/1.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'update', :id => '1', :format => 'xml')
      {:put  => '/<%= model_controller_routing_path %>/1.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'update', :id => '1', :format => 'json')
    end
    ...
    it "should generate params for <%= model_controller_controller_name %>'s destroy action from DELETE /<%= model_controller_routing_path %>/1" do
      {:delete => '/<%= model_controller_routing_path %>/1'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'destroy', :id => '1')
      {:delete => '/<%= model_controller_routing_path %>/1.xml'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'destroy', :id => '1', :format => 'xml')
      {:delete => '/<%= model_controller_routing_path %>/1.json'}.should route_to(:controller => '<%= model_controller_controller_name %>', :action => 'destroy', :id => '1', :format => 'json')
    end
  end
